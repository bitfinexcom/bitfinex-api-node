...
class OrderBook extends EventEmitter {
  ...
  checksum () {
    ...
    for (let i = 0; i < 25; i += 1) {
      if (this.bids[i]) {
        data.push(this.bids[i][0], this.bids[i][2])
      }

      if (this.asks[i]) {
        data.push(this.asks[i][0], this.asks[i][2])
      }
    }
  ...
  static checksumRaw (arr) {
    ...
    for (let i = 0; i < 25; i += 1) {
      ...
      if (bid) data.push(bid[0], bid[2])
      if (ask) data.push(ask[0], ask[2])
    }
   ...
   */
  midPrice () {
    const topAsk = (this.asks[0] || [])[0] || 0
    const topBid = (this.bids[0] || [])[0] || 0
