...
class Order extends Model {
  ...
  submit () {
    if (!this._ws) return Promise.reject(new Error('no ws connection'))

    return this._ws.submitOrder(this)
  }
  ...
  _onWSOrderUpdate (order) {
    const oCID = Array.isArray(order) ? order[2] : order.cid
    if (oCID !== this.cid) return
