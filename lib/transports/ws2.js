...
class WSv2 extends EventEmitter {
   ...
   * @private
   */
  _propagateMessageToListeners (msg) {
    const listenerGroups = Object.values(this._listeners)

    for (let i = 0; i < listenerGroups.length; i++) {
      WSv2._notifyListenerGroup(listenerGroups[i], msg, this._transform)
    }
  }
  ...
  static _notifyListenerGroup (lGroup, msg, transform) {
    ...
    WSv2._notifyCatchAllListeners(lGroup, payload)
    ...
    for (let j = 0; j < lGroup[eventName].length; j++) {
      ...
      if (filter && !WSv2._payloadPassesFilter(payload, filter)) {
        continue
      }
  ...
  _flushOrderOps () {
    ...
    this.submitOrderMultiOp(packets)
  }
