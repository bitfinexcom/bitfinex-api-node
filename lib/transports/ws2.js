...
const FLAGS = {
  ...
  CHECKSUM: 131072  // enable checksum per OB change, top levels 25 per-side
}
...
class WSv2 extends EventEmitter {
  ...
  _handleOBChecksumMessage (msg, chanData) {
    ...
    const err = this._verifyManagedOBChecksum(symbol, cs)
  ...
  _handleOBMessage (msg, chanData) {
    ...
    if (this._transform) {
      data = new OrderBook(Array.isArray(data[0]) ? data : [data])
    }
   ...
   */
  _verifyManagedOBChecksum (symbol, cs) {
