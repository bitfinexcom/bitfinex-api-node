...
class RESTv2 {
   ...
   * @private
   */
  _makePublicPostRequest (path, body, modelClass) {
    const url = `${this._url}/v2${path}`
  ...
  ticker (symbol = 'tBTCUSD', cb) {
    return this._makePublicRequest(`/ticker/${symbol}`, cb, TradeTicker)
  }
  ...
  tickers (symbols = [], cb = () => {}) {
    ...
    return this._makePublicRequest(`/tickers?symbols=${symbols.join(',')}`, cb, TradeTicker)
  }
