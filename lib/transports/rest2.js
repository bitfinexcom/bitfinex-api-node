...
class RESTv2 {
   ...
   * @return {Arr} merged arr of currencies and names sorted with no pairs repeated
   * @private
   */
  ...
  _mergeCurrenciesListSymLabel (data) {
    ...
    const listedCurr = data[0]
    const mapedCurrSym = data[1]
    const mapedCurrLabel = data[2]
    const listedCurrAsArr = []

    listedCurr.forEach((c) => {
      listedCurrAsArr.push([ c, c ])
    })
    ...
    const indexNotInArr = (arr, c) => {
      return arr.every((el) => {
        return el[0] !== c[0]
      })
    }

    const allNotRepeatedCurr = []
    allCurr.forEach((c) => {
      if (indexNotInArr(allNotRepeatedCurr, c)) {
        allNotRepeatedCurr.push(c)
      }
    })

    allNotRepeatedCurr.sort((a, b) => {
      return a[0].localeCompare(b[0])
    })

    return allNotRepeatedCurr
  }
