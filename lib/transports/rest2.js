...
class RESTv2 {
  ...
  /**
   * @param {Arr} data order matters
   * @return {Arr} merged arr of coins and names with no pairs rpeated
   * @private
   */
  _mergeAllArrOfCoins (data) {
    const addObjByKey = (obj, curr) => {
      if (!Array.isArray(curr)) obj[curr] = curr
      else if (curr.length === 1) obj[curr[0]] = curr[0]
      else obj[curr[0]] = curr[1]

      return obj
    }

    const arrOfKeyObjects = _transform(data, (allObjects, arr, ii) => {
      const keyObjects = _transform(arr, (tempObj, curr, i) => {
        return addObjByKey(tempObj, curr)
      }, {})

      return allObjects.push(keyObjects)
    }, [])

    const merge = _assign({}, ...arrOfKeyObjects)
    const res = _transform(merge, (result, value, index) => {
  ...
  currencies (cb = () => {}) {
    ...
    const url = `/conf/pub:list:currency${suffix},pub:map:currency:sym${suffix},pub:map:currency:label${suffix}`
    return this._makePublicRequest(url, cb, (data) => {
      const res = this._mergeAllArrOfCoins(data)
