...
class RESTv2 {
   ...
   * @param {Object?} modelClass - optional
   * @return {Promise} p - use this
   */
  _makePublicLegacyRequest (method, cb = () => {}, modelClass) {
    return new Promise((resolve, reject) => {
      this._rest1.make_public_request(method, (err, data) => {
        ...
        const out = modelClass ? this._doTransform(modelClass, data) : data
        cb(null, out)
        resolve(out)
      })
    })
   ...
   * @return {Promise} p
    */
  _makeAuthLegacyRequest (method, params = {}, cb = () => {}, modelClass) {
    return new Promise((resolve, reject) => {
  ...
  volumeStats (symbol = '', cb = () => {}) {
    ...
    return this._makePublicLegacyRequest(`stats/${symbol}`, cb)
  }
